...
The AI-Driven Development Lifecycle (AI-DLC) is a software development methodology introduced by Raja SP, Principal Solutions Architect and Head of Developer Transformation Programs at Amazon Web Services, in the AWS DevOps blog post "AI-Driven Development Life Cycle: Reimagining Software Engineering" on 31 July 2025.
...
There is one naming caveat. "ADLC" is also used elsewhere for "Agent Development Lifecycle" and related agent lifecycle frameworks, and separately AWS also uses "AI Development Life Cycle" for an unrelated ML model-building framework tied to the AWS AI Practitioner certification. This post is about Raja SP's software-delivery AI-DLC, not those other frameworks.
...
AWS says both patterns have produced suboptimal outcomes in velocity and quality. AI-DLC is the proposed middle path: AI performs heavy execution work while humans supply business context, judgment, validation and accountability. At AWS re:Invent 2025, Raja SP and Anupam Mishra presented this same diagnosis, drawn from more than 100 customer engagements, and reported productivity gains in the 10-15x range with customers including Wipro.
...
The AWS version describes three phases: Inception, Construction and Operations. The third-party AI-DLC 2026 paper keeps Inception and Operations but often uses "Execution" for the build phase. The intent is the same: move from clarified intent to verified implementation and then to operational ownership.
...
Construction uses the validated context from Inception to produce architecture, domain models, code solutions and tests. In the 2025 AWS framing this is Mob Construction. In the 2026 community paper this is commonly described as Execution through Bolts.
...
The third-party AI-DLC 2026 paper introduces three operating modes. They form a spectrum of human involvement rather than a maturity ladder.
...
Instead of prescribing every implementation step, the 2026 community paper emphasizes quality gates that reject bad output. A team does not need to tell AI "create interface, implement class, then write unit tests." It can define constraints: tests pass, type checks succeed, linting is clean, security scans clear and coverage exceeds a threshold.
...
The third-party AI-DLC 2026 paper introduces Passes: typed iterations through elaborate, decompose, execute and review. Built-in pass types include design, product and dev. Pass-backs let later passes return work to earlier passes when new constraints appear. Note that within the Bushido Collective's own ecosystem, Passes are already being superseded by a "stages" model as part of their broader H·AI·K·U framework.
...
In short: AI-DLC is a strong lifecycle model for AI-native software delivery, but it is not a substitute for engineering judgment, governance or measurable verification. It's also worth remembering that the fuller, more operational version of the methodology (modes, passes, quality gates) comes from an independent community project, not from AWS, so teams evaluating it for enterprise use should weigh that provenance accordingly.

## Getting Started

Raja SP outlines paths for adopting the official AWS version of AI-DLC:

1. Read the AWS DevOps blog post and the AI-DLC Method Definition Paper it links to
2. Use the official open-source reference implementation at `github.com/awslabs/aidlc-workflows` (MIT-0 licensed, supports Amazon Q Developer, Kiro, Cursor, Cline and others)
3. Explore Amazon Q Developer rules and Kiro custom workflows
4. Work with an AWS account team to adapt the method to organizational needs
...
- AWS DevOps Blog — [Open-Sourcing Adaptive Workflows for AI-DLC](https://aws.amazon.com/blogs/devops/open-sourcing-adaptive-workflows-for-ai-driven-development-life-cycle-ai-dlc/), 30 November 2025
- AWS Labs — [aidlc-workflows](https://github.com/awslabs/aidlc-workflows), official open-source reference implementation
- The Bushido Collective — [AI-DLC 2026 Paper](https://ai-dlc.dev/paper), 21 January 2026 (independent community project, not an AWS publication)
