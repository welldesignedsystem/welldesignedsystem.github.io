...
> **Where does the 60% figure come from?** Three production-team reports converge on this range: [FutureAGI (Feb 2026)](https://futureagi.com/blog/deterministic-llm-evaluation-metrics-2026/) reports deterministic checks catch *30 to 60 percent of failures before any LLM judge fires*. The [PIAS agent-eval harness (Apr 2026)](https://fde10x.com/t/llm-agent-eval-harness) reports a 70/30 deterministic-to-judge split across five production agents, noting *"your repo will land between 60/40 and 80/20 depending on how much of the workload is text generation versus tool use."* The [G-Eval production guide (Mar 2026)](https://futureagi.com/blog/g-eval-definitive-guide-2026/) recommends routing every response through a deterministic floor (schema, regex, length, banned phrases) before any LLM judge call, dropping the judge bill 80–90% without losing detection rate on the cases that genuinely need reasoning. The consensus: deterministic checks are the cheapest, fastest, and most reliable layer — they should be the base of every eval pyramid.
